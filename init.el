;;; Define Macro
(defmacro qv/load (modules &rest exprs)
  (declare (indent 1))
  `(let* ((qv/modules (if (listp ',modules) (car ',modules) ',modules))
          (qv/path (if (eq qv/modules '~) nil
                     (if (and (boundp 'qv/path) qv/path)
                         (append qv/path (list qv/modules)) (list qv/modules))))
          (qv/val qv/environment))
     (dolist (mod qv/path) (setq qv/val (alist-get mod qv/val)))
     (when (and (car qv/val) (not (equal (alist-get 'enable qv/val) '(nil))))
       ;;(ignore-errors
       ,(if (and (listp modules) (> (length modules) 1))
            `(qv/load ,(cdr modules) . ,exprs)
          `(progn . ,exprs))))) ;;)

(let* ((config-name
        (car (split-string
              (shell-command-to-string
               "cat ~/.emacs.d/configurations/current-configuration")
              "\n")))
       (config-file (format "~/.emacs.d/configurations/%s.el" config-name)))
  (if (file-regular-p config-file)
      (load config-file)
    (load (expand-file-name "~/.emacs.d/configurations/default.el"))))

(qv/load functions
  (qv/load run-shell-command
    (defun qv/run-in-background (command)
      (interactive "MSystem Command: ")
      (let ((command-parts (split-string command "[ ]+")))
        (apply #'call-process `(,(car command-parts) nil 0 nil ,@(cdr command-parts))))))

  (qv/load face-function
...
(qv/load packages
  ...
  (qv/load evil
    ...
    (qv/load global-movements
      (global-set-key (kbd "M-l") 'forward-char)
      (global-set-key (kbd "M-h") 'backward-char)
      (global-set-key (kbd "M-j") 'next-line)
      (global-set-key (kbd "M-k") 'previous-line))

    (qv/load tetra-movements
      (evil-define-motion qv/forward-four (count) (evil-forward-char (* 4 (or count 1))))
      (evil-define-motion qv/backward-four (count) (evil-backward-char (* 4 (or count 1))))
      (evil-define-motion qv/down-four (count) (next-line (* 4 (or count 1))))
      (evil-define-motion qv/up-four (count) (previous-line (* 4 (or count 1))))
    ...
    (qv/load one-key-deletions
      ...
      (defun qv/delete-backward-char ()
        (interactive)
        (delete-region (point) (max (1- (point)) (line-beginning-position))))
    ...
    (qv/load deleting-without-saving
    ...
    (qv/load meta-q-to-quit
      (global-set-key (kbd "M-q") 'evil-force-normal-state))

    (qv/load custom-search-mappings
  ...
  (qv/load helpful
    ...
    (qv/load format-helpful-buffers
  ...
  (qv/load ivy
    ...
    (qv/load faces
      (defun qv/minibuffer-faces ()
        (qv/face 'minibuffer-prompt nil qv/blue-color nil
                 :height 1.2
                 :weight 'bold)
        (qv/face 'ivy-current-match 'highlight nil 0)
      ...
      (add-hook 'minibuffer-setup-hook 'qv/minibuffer-faces))
    ...
    (qv/load details
      ...
      (setq qv/detail-function-alist
            `((helpful-variable . (lambda (v) (documentation-property
              ...
              ;;(helpful-callable . helpful--signature)
              (helpful-callable . documentation)
              (execute-extended-command . helpful--signature)))
  ...
  (qv/load embark
    ...
    (add-hook 'minibuffer-setup-hook
              (lambda () (local-set-key (kbd "C-a") 'embark-act)))
  ...
  (qv/load rainbow-delimiters
    ...
    (defun qv/rainbow-delimiters-faces ()
    ...
    (add-hook 'rainbow-delimiters-mode-hook 'qv/rainbow-delimiters-faces)))

(qv/load settings
  (qv/load unmap-ctrl-z
    (global-unset-key (kbd "C-z")))

  (qv/load caps-lock-control
    (qv/run-in-background
     (format "xmodmap <(%s\n%s\n%s)"
             "remove Lock = Caps_Lock"
             "keycode 0x42 = Control_L"
             "add Control = Control_L")))
  (qv/load display-buffer
    (setq display-buffer-base-action '(display-buffer-below-selected ())))

  (qv/load show-time
    (setq display-time-format (car qv/val))
    (display-time-mode 1))

  (qv/load theme-directory
    (setq custom-theme-directory "~/.emacs.d/themes"))

  (qv/load disable-gui-elements
    (scroll-bar-mode -1)
    (tool-bar-mode -1)
    (menu-bar-mode -1)
    (tooltip-mode -1))

  (qv/load disable-start-screen
    (setq-default inhibit-startup-message t))

  (qv/load subword-movements
    (global-subword-mode 1))

  (qv/load enable-trash
    (setq-default delete-by-moving-to-trash t))

  (qv/load configure-auto-saves
  ...
  (qv/load disable-auto-saves
    (setq-default auto-save-default nil))

  (qv/load disable-backup-files
    (setq-default make-backup-files nil))

  (qv/load disable-lock-files
  ...
  (qv/load tab-width
    (setq-default tab-width qv/val))

  (qv/load show-line-numbers
    (global-display-line-numbers-mode t))

  (qv/load enable-relative-window-resizing
    (setq window-combination-resize t))

  (qv/load recursive-minibuffers
    (setq enable-recursive-minibuffers t))

  (qv/load winner-mode
    (winner-mode))

  (qv/load set-scroll-margin
    (setq-default scroll-margin (car qv/val))
    (setq-default maximum-scroll-margin 1.0))

  (qv/load set-scroll-step
    (setq-default scroll-step (car qv/val)))

  (qv/load stretch-cursor
    (setq-default x-stretch-cursor t))

  (qv/load fringe-width
    (set-fringe-mode (car qv/val)))

  (qv/load remember-recent-files
    (recentf-mode 1)
...
(qv/load org
  ...
  (qv/load faces
    (defun qv/org-faces ()
      ...
      (qv/face 'org-ellipsis nil nil nil
               :underline nil))
    (add-hook 'org-mode-hook 'qv/org-faces))

  (qv/load bullet-overlay
    (font-lock-add-keywords
     'org-mode
     '(("^ *\\([-]\\) "
        (0 (prog1 () (compose-region (match-beginning 1) (match-end 1) "•")))))))

  (qv/load hide-ellipsis
    (setq org-ellipsis " ")
    (qv/face org-ellipsis nil nil nil))

  (qv/load hide-emphasis-markers
    (setq org-hide-emphasis-markers t))

  (qv/load small-empty-lines
    (font-lock-add-keywords
     'org-mode
     '(("\\(^\n\\)" (1 '(:height 0.5))))))
  ...
  (qv/load indent-nested-headings
    (add-hook 'org-mode-hook 'org-indent-mode))

  (qv/load variable-pitch-font
  ...
  (qv/load disable-line-numbers
    (add-hook 'org-mode-hook (lambda () (display-line-numbers-mode 0))))


  (qv/load search-headings
    (add-hook 'org-mode-hook
              (lambda () (local-set-key (kbd "C-c /") 'counsel-outline))))

  (qv/load return-follows-links
    (add-hook
     'org-mode-hook
     (lambda ()
...
other formatting."
    ...
    (add-hook
     ...
     (lambda ()
       ...
       (evil-define-key 'visual 'local (kbd "M-`") 'qv/visual-equation)
...
for the device with id or name of DEVICE"
  ...
  (qv/load display-settings
    (qv/run-in-background "xrandr -s 1920x1080")
    (qv/run-in-background "xgamma -gamma 1.2")))

(qv/load programming
  (qv/load enable
    (add-hook 'prog-mode-hook 'qv/prog-mode-setup)

    (defun qv/prog-mode-setup ()
      (buffer-face-set nil 'fixed-pitch)))

  (qv/load outline-mode
    (defun qv/outline-mode-setup ()
      (outline-minor-mode)
      (setq-local outline-level 'outline-level)

      (if (string= (buffer-file-name) (expand-file-name "~/.emacs.d/init.el"))
          (setq-local outline-regexp "^ *;;;.\\{5\\}\\|^ *(qv/load")
        (setq-local outline-regexp "^;;;\\|^ *(.\\{3\\}\\|^ *;.\\{3\\}"))

      (evil-define-key 'normal 'local (kbd "SPC") 'outline-toggle-children)
      (evil-define-key 'normal 'local (kbd "gzr") 'outline-show-all)
      (evil-define-key 'normal 'local (kbd "gzj") 'outline-next-heading)
      (evil-define-key 'normal 'local (kbd "gzk") 'outline-previous-heading)
      (evil-define-key 'normal 'local (kbd "gzm")
        (lambda (level) (interactive "P")
          (save-excursion
            (beginning-of-buffer)
            (outline-hide-sublevels
             (+ 8 (if level (if (numberp level) (* 2 level) 46) 0))))))
      (evil-define-key 'normal 'local (kbd "gzs") 'outline-show-subtree)
      (evil-define-key 'normal 'local (kbd "gzh")
        (lambda () (interactive)
          (when (eq (outline-back-to-heading) t)
            (outline-up-heading 1))
          (outline-hide-subtree)))
      (evil-define-key 'normal 'local (kbd "gzl")
        (lambda () (interactive)
          (outline-show-entry)
          (outline-show-children)
          (next-line))))

    (add-hook 'prog-mode-hook 'qv/outline-mode-setup))

  (qv/load elisp-setup
    (defun qv/elisp-setup ()
      (font-lock-add-keywords
       'emacs-lisp-mode
    ...
    (add-hook 'emacs-lisp-mode-hook 'qv/elisp-setup))

  (qv/load adding-emacs-settings
    (global-set-key (kbd "<f6>") 'qv/add-setting)

    (defun qv/add-setting (setting value)
      (interactive
       (list (read-string "Setting Name: ")
             (read-string "Default Value: " "t")))
      (with-selected-window (get-buffer-window "init.el")
        (evil-open-below) (newline)
        (let ((start (point)))
          (insert (format "(qv/load %s " setting))
          (newline) (yank) (kill-whole-line)
          (previous-line) (end-of-line)
          (insert ")")
          (indent-region start (point))))
      (with-selected-window (get-buffer-window "default-theme.el")
        (end-of-line) (newline)
        (insert (format "(%s %s)" setting value))
...
(qv/load extra
  (qv/load basic-faces
    ;;; Base faces
    (qv/face 'default nil qv/fg-color
             (if (display-graphic-p) qv/bg-color nil)
    ...
    (qv/face 'highlight nil qv/yellow-color 0
             :weight 'semibold
             :underline qv/yellow-color)
  ...
  (qv/load browse-kill-ring
    (when (qv/load (~ minibuffer prescient))
  ...
  (qv/load change-text-size
    ...
    (global-set-key (kbd "C-+")
                    (lambda () (interactive)
                      (change-font-size 'default 8)))
    (global-set-key (kbd "C-_")
                    (lambda () (interactive)
                      (change-font-size 'default -8))))

  (qv/load custom-buffers
    (defun qv/custom-mode-setup ()
    ...
    (add-hook 'custom-mode-hook 'qv/custom-mode-setup)
