...
(setq bz/jsx-major-mode 'web-mode)
...
(let* ((snippets
        '(("C" (lambda ()
                 (let* ((n (if (eq (point-min) (point-max))
                               (let* ((file (file-name-base buffer-file-name)))
                                 (if (not (equal file "index")) file
                                   (file-name-base (f-parent buffer-file-name))))
                             (read-string "Component name: "))))
                   (insert (format "export type %sProps = PropsWithChildren<{\n    \n}>\n\n" n))
                   (insert (format "export default function %s(p: %sProps) {\n    return <div></div>;\n}" n n))
                   (search-backward "\n}>"))))
          ...
          ("i l" "import * as Icons from \"lucide-react\";")

          ("c" "className=\"<<>>\"")

          ("r d" "<div><<>></div>")
          ("r D" "<div>\n<<>>\n</div>")
          ("r c" "<div className=\"<<>>\"></div>")
          ("R" "<<<tag>>>\n<<>>\n</<<tag>>>")
  ...
  (setf (alist-get '(js-jsx-mode web-mode) bz/snippet-mode-alist nil nil #'equal) snippets))
